...
class ReweightOODTrainer:
    ...
    def train_epoch(self, epoch_idx):
        ...
        for train_step in tqdm(range(1,
            ...
            sp_matrix = label_matrix.triu(diagonal=1)
            sn_matrix = label_matrix.logical_not().triu(diagonal=1)
            s_matrix = feature @ feature.transpose(1, 0)
            s_matrix = s_matrix.view(-1)
            sp_matrix = sp_matrix.view(-1)
            sn_matrix = sn_matrix.view(-1)
            sp, sn = s_matrix[sp_matrix], s_matrix[sn_matrix]
            wp = torch.sigmoid(-(sp.detach() * self.m_w - self.c_w))
            wn = torch.sigmoid(  sn.detach() * self.m_b - self.c_b)
            loss = self.elu(torch.logsumexp( wn * sn / self.temp, dim=0) +
                            torch.logsumexp(-wp * sp / self.temp, dim=0))
